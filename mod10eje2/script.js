...
function validarCampo(input) {
    // Crear o localizar el span de error para asegurar robustez
    ...
    if (isNaN(value)) {
        errorSpan.textContent = 'Ingresa un número válido';
        input.classList.add('invalid');
        input.classList.remove('valid');
        input.setAttribute('aria-invalid', 'true');
        return false;
...
pesoInput.addEventListener('input', () => { validarCampo(pesoInput); updateCalculateState(); });
alturaInput.addEventListener('input', () => { validarCampo(alturaInput); updateCalculateState(); });
edadInput.addEventListener('input', () => { validarCampo(edadInput); updateCalculateState(); });

function updateCalculateState() {
    // No forzar mensajes adicionales: validarCampo es suficiente para mostrar estado
    const ok = validarCampo(pesoInput) && validarCampo(alturaInput) && validarCampo(edadInput);
    if (calculateBtn) calculateBtn.disabled = !ok;
}
...
form.addEventListener('submit', (e) => {
    ...
    if (!pesoValido || !alturaValida || !edadValida) {
        // Llevar foco al primer inválido para mejor UX
        const firstInvalid = [pesoInput, alturaInput, edadInput].find(i => i.classList.contains('invalid'));
