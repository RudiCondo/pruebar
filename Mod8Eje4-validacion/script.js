...
function validateField(field) {
    ...
    switch(field.id) {
        ...
        const startsWithZeroRegex = /^0\d+$/; // Coincide con una cadena que empieza con 0 y es seguida por uno o más dígitos.
            
        if (value.length === 0) {
            message = 'La edad es obligatoria';
            isValid = false;
        } else if (startsWithZeroRegex.test(value)) {
            // Nueva verificación: Si la cadena empieza con 0 y tiene más de un dígito
            message = 'La edad no puede empezar con cero (ej. 028)';
            isValid = false;
